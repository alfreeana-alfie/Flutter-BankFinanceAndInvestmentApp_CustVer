...
void signIn(BuildContext context, Map<String, String> body) async {
  ...
  if (response.statusCode == 200) {
    var tokenBody = Token.fromJSON(jsonDecode(response.body));
...
void getUserDetails(String accessToken) async {
  final response = await http.post(API.loginUrl, headers: {
  ...
  if (response.statusCode == 200) {
    var tokenBody = User.fromJSON(jsonDecode(response.body));
    String accessToken = tokenBody;
