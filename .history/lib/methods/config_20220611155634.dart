...
class SMSNigeriaAPI {
  static void send(BuildContext context, String to, String body) async {
    Uri url = Uri.parse(smsApi.toString() +
        '?api_token=apYsrAtoGSjgkicEXuDyXDaIZfFQXjl3gQZ5AYVilVuqZq7TkWT6hSPfsaXD&from=BulkSMS.ng&to=$to&body=$body&dnd=6');
...
class Method {
  ...
  static void addFileBytes(
    ...
    final request = http.MultipartRequest(Field.postMethod, url)
      ..fields.addAll(body)
      ..headers.addAll(headersMultiPart)
      ..files.add(http.MultipartFile.fromBytes(image, listData, filename: 'attachment.pdf'));
  ...
  static void editFileBytes(
    ...
    final request = http.MultipartRequest(Field.postMethod, url)
      ..fields.addAll(body)
      ..headers.addAll(headersMultiPart)
      ..files.add(http.MultipartFile.fromBytes(image, listData, filename: 'wire_transfer.pdf'));
