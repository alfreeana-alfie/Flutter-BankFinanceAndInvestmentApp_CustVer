...
class _WebPageState extends State<WebPage> {
  ...
  void sendData() {
    ...
    if (widget.method == 'sms_subcribed') {
    ...
    if (widget.toUserId != null) {
      if (userLoad.emailVerifiedAt != null) {
        ...
        EmailJS.send(
            context,
            widget.toUserEmail ?? 'customer@gmail.com',
            widget.toUserName ?? 'user',
            'FVIS Investment',
            emailMessage ?? 'Default',
        ...
        SMSNigeriaAPI.send(context, widget.userPhone ?? '000', widget.message ?? 'default');
        SMSNigeriaAPI.send(context, widget.toUserId ?? '000', widget.messageTo ?? 'default');
      }
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => CardList(
                ...
                type: widget.type ?? Type.nullable,
                routePath: widget.routePath,
                pageName: widget.pageName ?? 'Page',
              ),
            ),
          );
        });
      ...
      if (userLoad.emailVerifiedAt != null) {
        ...
        SMSNigeriaAPI.send(context, widget.userPhone ?? '000', message ?? 'default');
      }
