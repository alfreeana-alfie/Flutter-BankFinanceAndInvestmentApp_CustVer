...
import 'package:flutter_banking_app/widgets/dropdown/dropdown.dart';
              ...
              child: Column(
                ...
                children: [
                  ...
                  NewField(
                    mandatory: true,
                    initialValue: widget.type == Field.update
                        ? widget.bank?.chargeInPercentage
                        : Field.empty,
                  ...
                  ToggleSwitch(
                    initialLabelIndex: int.parse(
                        widget.bank?.status.toString() ?? Field.emptyInt),
                    minWidth: 120,
                    cornerRadius: 7.0,
                    ...
                    child: RoundedLoadingButton(
                      ...
                      onPressed: () {
                        ...
                        Map<String, String> body = {
                          Field.name:
                              name ?? widget.bank?.name ?? Field.emptyString,
                          Field.swiftCode: swiftCode ??
                              widget.bank?.swiftCode ??
                              Field.emptyInt,
                          Field.bankCountry: bankCountry ??
                              widget.bank?.bankCountry ??
                              Field.emptyString,
                          Field.bankCurrency: currency ??
                              widget.bank?.bankCurrency.toString() ??
                              Field.empty,
                          Field.minTransferAmt: minTransferAmt ??
                              widget.bank?.minTransferAmt ??
                              Field.emptyAmount,
                          Field.maxTransferAmt: maxTransferAmt ??
                              widget.bank?.maxTransferAmt ??
                              Field.emptyAmount,
                          Field.fixedCharge: fixedCharge ??
                              widget.bank?.fixedCharge ??
                              Field.emptyAmount,
                          Field.chargeInPercentage: chargeInPercentage ??
                              widget.bank?.chargeInPercentage ??
                              Field.emptyAmount,
                        ...
                        // OtherBankMethods.edit(context, body, widget.bank?.id.toString() ?? Field.emptyInt);
                        widget.type == Field.update
                            ? Method.edit(
                                context,
                                _btnController,
                                body,
                                Uri.parse(AdminAPI.updateOtherBank.toString() +
