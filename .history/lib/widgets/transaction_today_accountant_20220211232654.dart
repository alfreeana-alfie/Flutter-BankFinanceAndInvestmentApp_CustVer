...
class _TransactionTodayListAccountantState extends State<TransactionTodayListAccountant> {
  ...
  List<UserWallet> transactionListNew = [];

  void getCurrency() async {
    final response = await http.get(AdminAPI.listOfUserWalletsAccountant, headers: headers);

    if (response.statusCode == Status.ok) {
      var jsonBody = jsonDecode(response.body);

      for (var data in jsonBody[Field.data]) {
        final transaction = UserWallet.fromMap(data);
