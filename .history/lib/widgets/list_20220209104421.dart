...
class _CardListState extends State<CardList> {
  ...
  Widget _card() {
    switch (widget.type) {
      ...
      case Type.faq:
        return Expanded(
          child: _foundQ.isNotEmpty
              ? ListView.builder(
                  itemCount: _foundQ.length,
                  itemBuilder: (context, index) => Card(
                      key: ValueKey(_foundQ[index].id),
                      color: Styles.transparentColor,
                      elevation: 0.0,
                      margin: const EdgeInsets.symmetric(vertical: 10),
                      child: CardFaq(
                        question: _foundQ[index],
                        questionList: _foundQ,
                        index: index,
                      )),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return CardFaq(
                      question: depositList[index],
                      questionList: depositList,
                      index: index,
                    );
                  },
                ),
