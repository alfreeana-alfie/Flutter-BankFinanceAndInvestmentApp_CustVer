...
class _CardListState extends State<CardList> {
  ...
  Widget _card() {
    switch (widget.type) {
      ...
      case Type.navigationItem:
        return Expanded(
          child: _foundNavitme.isNotEmpty
              ? ListView.builder(
                  itemCount: _foundNavitme.length,
                  itemBuilder: (context, index) => Card(
                      key: ValueKey(_foundNavitme[index].id),
                      color: Styles.transparentColor,
                      elevation: 0.0,
                      margin: const EdgeInsets.symmetric(vertical: 10),
                      child: CardNavigation(
                        navigation: _foundNavitme[index],
                        navigationList: _foundNavitme,
                        index: index,
                      )),
                )
              : ListView.builder(
