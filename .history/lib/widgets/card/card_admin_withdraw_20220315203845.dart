...
class CardWithdrawAdmin extends StatefulWidget {
  const CardWithdrawAdmin({Key? key,
      ...
      this.userType,
      this.customerId,
      this.currentRate, this.method, this.userId}) : super(key: key);

  final Withdraw withdraw;
  final List<Withdraw> withdrawList;
  final int index;
  final int? currentRate;
  final String? userType, customerId, userId;
  final String? method;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Column(
          ...
          children: [
            ...
            DetailRow(
                labelTitle: Str.purpose,
                labelDetails: widget.withdraw.purpose ?? Field.emptyString),
            DetailRow(
                labelTitle: Str.approved,
                labelDetails: widget.withdraw.approved ?? Field.emptyString),
            DetailRow(labelTitle: Str.created, labelDetails: createdAt),
            // DetailRow(labelTitle: Str.countryCode, labelDetails: withdraw.countryCode ?? Field.emptyString),
            _buildButtonRow(context),
          ],
      ...
      children: [
        Expanded(
          child: RoundedLoadingButton(
            ...
            onPressed: () {
              Map<String, String> body = {'approved': 'approved'};
              double newAmount = double.parse(widget.withdraw.amount!);
              setState(() => newAmount *= double.parse(widget.withdraw.exchangeRate!));

              double rateCharge = newAmount;
              setState(() =>
                  rateCharge *= (double.parse(widget.currentRate.toString()) / 100));
              ...
              // Calculation - Wallet subtract Updated Amount
              double updatedBalance = double.parse(widget.withdraw.walletBalance!);
              ...
              Map<String, String> bodyWallet = {
                Field.userId: widget.withdraw.customerId ?? Field.emptyInt,
                Field.walletId: widget.withdraw.walletId ?? '0',
                Field.accountId: '1',
                Field.method: 'withdraw',
                Field.creditDebit: 'credit',
                Field.amount: double.parse(widget.withdraw.amount!).toStringAsFixed(2),
                Field.rateAmount: rateCharge.toStringAsFixed(2),
                Field.grandTotal: updatedAmount.toStringAsFixed(2),
                Field.paymentMethod: 'Withdrawal',
                Field.currentRate: widget.currentRate.toString(),
                Field.updatedBy: widget.userId ?? '0',
                Field.currencyId: widget.withdraw.currencyId ?? '10',
                Field.transactionCode: Field.transactionCodeInitials +
                    '${widget.withdraw.currency}-' +
                    getRandomCode(6)
              };

              Map<String, String> updateWalletBody = {
                Field.amount: updatedBalance.toString(),
                Field.updatedBy: widget.userId ?? '3',
              };

              WalletMethods.addTransaction(context, bodyWallet);
              WalletMethods.update(
                  context, updateWalletBody, widget.withdraw.walletId ?? '0');

              print(widget.withdraw.exchangeRate);
              print(widget.currentRate);
              print(updatedAmount);
              print(updatedBalance);
